# ThinkOrSwim NASDAQ Previous High/Low Analysis - Simplified Working Version
...
input lookback_period = 20;  # Days to look back for statistics
...
def within_60min = time_since_open <= 60;
...
# Calculate historical statistics from actual data
def high_continuation_10min_avg = Average(continuation_move_10min_high, lookback_period);
def high_reversal_10min_avg = Average(reversal_move_10min_high, lookback_period);
def low_continuation_10min_avg = Average(continuation_move_10min_low, lookback_period);
def low_reversal_10min_avg = Average(reversal_move_10min_low, lookback_period);
...
# Plot averages
plot(high_continuation_10min_avg, "High Continuation Avg", Color.ORANGE, 1);
plot(high_reversal_10min_avg, "High Reversal Avg", Color.PINK, 1);
plot(low_continuation_10min_avg, "Low Continuation Avg", Color.CYAN, 1);
plot(low_reversal_10min_avg, "Low Reversal Avg", Color.MAGENTA, 1);
