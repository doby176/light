#!/usr/bin/env python3
"""
NASDAQ Previous High/Low Analysis Chart Generator
Generates comprehensive charts from CSV data for trading analysis
...
def create_overall_statistics_chart(df, output_dir):
    """Create overall statistics comparison chart"""
    print("Generating overall statistics chart...")
...
def create_position_analysis_chart(df, output_dir):
    """Create analysis by market position at open"""
    print("Generating position analysis chart...")
...
def create_day_of_week_chart(df, output_dir):
    """Create day of week analysis chart"""
    print("Generating day of week analysis chart...")
    ...
    plt.tight_layout()
    output_path = os.path.join(output_dir, 'day_of_week_analysis.png')
    plt.savefig(output_path, dpi=300, bbox_inches='tight')
    plt.close()
    print(f"Saved: {output_path}")
...
def create_time_series_chart(df, output_dir):
    ...
    plt.tight_layout()
    output_path = os.path.join(output_dir, 'time_series_analysis.png')
    plt.savefig(output_path, dpi=300, bbox_inches='tight')
    plt.close()
    print(f"Saved: {output_path}")
...
def create_summary_chart(df, output_dir):
    ...
    print("Generating trading summary chart...")
    ...
    plt.tight_layout()
    output_path = os.path.join(output_dir, 'trading_summary.png')
    plt.savefig(output_path, dpi=300, bbox_inches='tight')
    plt.close()
    print(f"Saved: {output_path}")
...
def main():
    ...
    if not os.path.exists(csv_path):
        ...
        print("Please update the 'csv_path' variable in the script to point to your CSV file.")
        return
    ...
    try:
        ...
        # Generate all charts
        print("\nGenerating charts...")
        create_overall_statistics_chart(df, output_dir)
        create_position_analysis_chart(df, output_dir)
        create_day_of_week_chart(df, output_dir)
        create_time_series_chart(df, output_dir)
        ...
        print("\nTroubleshooting:")
        print("1. Make sure your CSV file exists and has the correct format")
        print("2. Install required packages: pip install pandas matplotlib seaborn numpy")
        print("3. Check that your CSV has these columns: date, touch_type, continuation_move_pct, reversal_move_pct, continuation_move_pct_60min, reversal_move_pct_60min, open_position, day_of_week")
