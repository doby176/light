...
def shouldGoLong = currentPosition == 0 and isBullishOrderBlock and bullishUnmitigated;

# Rule 2: Exit long and go short on red dot (close below Bullish Order Block)
def shouldExitLongGoShort = currentPosition == 1 and closeBelowBullish and !redDotPlotted[1];

# Rule 3: Exit short and go long when price hits green dot level (lastBullishLevel)
def shouldExitShortGoLong = currentPosition == -1 and high1 >= lastBullishLevel;
...
def newEntryPrice = if shouldGoLong then close1
                    else if shouldExitLongGoShort then close1
                    else if shouldExitShortGoLong then lastBullishLevel
                    else entryPrice;

def newStopLossPrice = if shouldGoLong then close1 * 0.99
                       else if shouldExitLongGoShort then lastBullishLevel
                       else if shouldExitShortGoLong then lastBullishLevel * 0.99
                       else stopLossPrice;
...
# --- Performance Labels ---
AddLabel(yes, "Position: " + (if newPosition == 1 then "LONG" else if newPosition == -1 then "SHORT" else "FLAT"), 
         if newPosition == 1 then Color.GREEN else if newPosition == -1 then Color.RED else Color.GRAY);
AddLabel(yes, "Entry: $" + Round(newEntryPrice, 2), Color.WHITE);
AddLabel(yes, "Stop: $" + Round(newStopLossPrice, 2), Color.RED);
